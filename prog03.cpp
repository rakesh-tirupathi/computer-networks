...
//              server and echo back to the original browser. It also illustrates how
//				how the server uses thread/fork to implement several processes/threads
...
main( int argc, char *argv[])
{
   ...
   int sock1fd, newsock1fd,          // file descriptor of sockets
       ...
       addrlen, client_addrlen, cnt=0, pid;    // address length
   ...
   while(1)
   {
	   ...
	   cnt++;     // increment the number of services
	   /*if (newsock1fd < 0)
	   { 
        ...
        if (pid <0)
        { 
             cerr << "Server: fork error: " << errno << "\n"; 
             exit(3); 
        }   
        else if (pid == 0) //child
		{
			//close(sock1fd);  // close  welcome socket	 */  
	   ...
	   if (newsock1fd < 0)
		...
		else if (pid == 0) //child
		{
		   ...
		   while(1)
		   {
			   ...
			   cout << "doing something before reply back ... \n";
			   sleep(10);
		...
		/*else // parent
		{
			close(newsock1fd);  //close new socket.
		}*/
		   ...
		   //while(sum<retcode1)
			...
			if(infile)
			{
				...
				while(!infile.eof())
				{
					...
					if (nread < 0)
					{
					   cerr << "Relay server's send failed...Server: write error: " << nread << "\n";
					   exit(3); 
					}
					
					cout << "  done replying to (" << cnt << ")....\n\n";
				}
			}
			else
			{
